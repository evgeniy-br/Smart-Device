...
const modalCall = document.querySelector('.modal-call__container');
...
if (modalCall) {
  orderButton.addEventListener('click', (evt) => {
    evt.preventDefault();
    modalCall.classList.toggle('modal-call__container--open');
  ...
  document.addEventListener('keydown', (evt) => {
    if (evt.key === 'Escape') {
      if (modalCall.classList.contains('modal-call__container--open')) {
        modalCall.classList.remove('modal-call__container--open');
      }
  ...
  window.addEventListener('click', (evt) => { // при клике в любом месте окна браузера
    ...
    if (!target.closest('.modal-call__container') && !target.closest('.header__contacts-button-link')) { // если этот элемент или его родительские элементы не модальное окно
      modalCall.classList.remove('modal-call__container--open'); // убираем класс открытой модалки
